    ...
    function _getAmountsIn(
        ...
        for (uint256 i = pairs.length; i != 0; i--) {
            ...
            if (version == Version.V1) {
                (uint256 reserMBOX, uint256 reserveOut,) = IJoePair(pair).getReserves();
                if (token > tokenPath[i]) {
                    (reserMBOX, reserveOut) = (reserveOut, reserMBOX);
                }

                uint256 amountOut_ = amountsIn[i];
                amountsIn[i - 1] = uint128(amountOut_.getAmountIn(reserMBOX, reserveOut));
